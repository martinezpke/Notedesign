...
const Modal = ({ title, description, onClick, data, id, updateData }) => {
  ...
  // funcion que cambia el estado de isSaveBtn
  const handleClick = () => {
  ...
  const updateNotaTime = async (idNota, title, description) => {
    ...
    try {
      const response = await updateNote(idNota, noteData);
      updateData()
  ...
  const debouncedUpdateNote = useMemo(() => {
    const debounceTimer = setTimeout(() => {
      updateNotaTime(id, valueTitle, valueDescription);
    }, 500); // el tiempo de espera

    return () => {
      clearTimeout(debounceTimer);
    };
  },[id, valueTitle, valueDescription]);
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(debouncedUpdateNote);
    };
  }, [debouncedUpdateNote]);
